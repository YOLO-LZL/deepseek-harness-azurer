...
- **v0 logs are history-only** — `inspect()` and `readFrom()` expose a v1 in-memory view without changing the artifact, but `prepare()`, `load()`, and live adoption reject v0 because its header has no execution location. Other non-current versions still refuse by direction; unknown event types refuse unless marked `ignorable` in the envelope. The versioning mechanism is the [session-log-version-mechanism note](../../../.agents/notes/implemented/architecture/2026-08-10-session-log-version-mechanism.md).
