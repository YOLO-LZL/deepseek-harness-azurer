...
- **v0 日志仅限历史查看**：`inspect()` 和 `readFrom()` 会返回 v1 的内存视图而不改写工件，但 `prepare()`、`load()` 和活动会话接管会拒绝 v0，因为其 header 没有执行位置。其他非当前版本仍按方向拒绝；不认识的事件类型也会被拒绝，除非信封带 `ignorable` 标记。版本机制见 [session-log 版本机制 Agent Note](../../../.agents/notes/implemented/architecture/2026-08-10-session-log-version-mechanism.md)。
