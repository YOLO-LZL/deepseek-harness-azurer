...
附加提供方使用按界面归属的 list slot `conversation.hero.workspace.createMethod` 和 `sidebar.workspaces.createMethod`；每个提供方都会把同一创建方法注册到两处。目录流占用者或提供方方法任一存在即可启用 **添加工作区…**；仅有 SSH 方法的组合会直接打开已注册的方法。
