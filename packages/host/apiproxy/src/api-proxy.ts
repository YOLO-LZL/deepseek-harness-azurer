 ...
 * plugin-owned entries (`agent-loop`, `bash`, `web-search-deepseek`,
 * `ssh-connections`) are the host-plane sections the plugin configuration
 * page edits; a namespace absent here answers `settings-not-exposed` even
 * when its owner registered it, so adding a section to that page is a
...
const WEB_SETTINGS_NAMESPACES = [
  'agent-loop', 'shell', 'locale', 'permission', 'ui-conversation', 'ui-theme', 'web-search-deepseek', 'ssh-connections',
] as const
