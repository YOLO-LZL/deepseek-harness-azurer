...
- **Only v1 may load or resume** — a v0 JSONL artifact remains readable through detached `inspect()` and `readFrom()` history views without being rewritten; changing compression still requires a separate/fresh root or selecting the legacy raw mode.
