...
- **仅 v1 可加载或恢复**：v0 JSONL 工件仍可通过独立的 `inspect()` 与 `readFrom()` 历史视图读取，不会被改写；更改压缩仍需要独立/全新根，或选择遗留原始 mode。
